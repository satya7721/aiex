import { Submission, Answer, Question } from "@/types";
...
export function validateSubmissionAnswers(
  ...
  return (
    questions.every((q) => answeredQuestionIds.has(q.id)) &&
...
export function generateSubmissionFeedback(score: number, totalMarks: number) {
  ...
  if (percentage >= 90) {
    comments.push(
      "Excellent performance! You have demonstrated exceptional understanding of the subject matter."
    );
    actionItems.push("Focus on maintaining this high level of performance");
  } else if (percentage >= 75) {
    comments.push(
      "Very good performance. You show strong grasp of most concepts."
    );
    actionItems.push(
      "Review the few questions you missed to achieve excellence"
    );
    ...
    comments.push(
      "You need to work harder to improve your understanding of the subject."
    );
